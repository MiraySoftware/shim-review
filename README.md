...
https://github.com/MiraySoftware/shim.git@miray-15

-------------------------------------------------------------------------------
What patches are being applied and why:
-------------------------------------------------------------------------------
* Enable 32-on-64
    We use a 32bit variant of our operating system as fallback if the 64bit variant
    does not work correctly, so we need to support validation of 32bit PE files on 
    a 64bit UEFI.


* FIX 32-on-64 support
    Support for 32bit operating systems on 64bit hardware does not work
    correctly in Shim 15
    This patch add a missing check to correctly support this use case
    Needs ALLOW_32BIT_KERNEL_ON_X64 define to have any effect
...
https://github.com/MiraySoftware/build_shim_miray.git@master and use the 
'build_shim' script
When run the script will check out the miray-15 repository into a subdirectory,
copy the certificate and build shim
As a result you will get a shim_mirayx64.efi file and a cab file with the efi 
file renamed to BOOTX64.EFI (This was the way to send the file to Microsoft)

The sha256 checksum is stored in shim_mirayx64.cksum
